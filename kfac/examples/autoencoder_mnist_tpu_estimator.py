...
This script demonstrates training on TPUs using KFAC optimizer and updating the
damping parameter according to the Levenberg-Marquardt rule.

See third_party/tensorflow_kfac/google/examples/ae_tpu_estimator_xm_launcher.py
...
example) are that you must use high-precision matrix multiplications, and
iterations_per_loop = 1. (The latter requirement is likely due to a bug
somewhere.)
"""
...
# For now we have to set iterations_per_loop to 1 since the algorithm doesn't
# perform well on TPUs otherwise. This is currently being investigated.
# TODO(b/129464710): Fix this issue!
